...
with st.sidebar:
    ...
    if st.session_state.sc_token:
        try:
            ...
            if signals.body['ev.battery.level'].value is not None and signals.body['ev.battery.capacity'].value is not None:
                ...
                st.warning("⚠️ Certaines données ne sont pas supportées par ce véhicule simulé.")
                if signals.body['ev.battery.level'].value:
                    soc_init = int(signals.body['ev.battery.level'].value * 100)
        
        except Exception:
            st.warning("⚠️ Impossible de lire les données en direct (non supporté).")
    ...
    if show_manual_inputs:
        if st.session_state.sc_token is None:
            ...
            st.link_button("🔗 Connecter mon véhicule", auth_url, use_container_width=True)
    ...
    if st.session_state.sc_token:
        if st.button("🔌 Déconnecter le véhicule", use_container_width=True):
            st.session_state.sc_token = None
            st.rerun()

    st.divider()
    soc_target = st.slider("Batterie arrivée visée (%)", 0, 100, 20)
    btn_calcul = st.button("🚀 Calculer l'itinéraire", use_container_width=True)
...
if st.session_state.calcul_fait:
    ...
    if res['needs_stop']:
        ...
        st.success(f"📍 **Plan de recharge :** Arrêt à la borne **{res['borne']['nom'].strip()}** pendant **{res['t_charge']:.0f} minutes**.")
    else:
        cols[2].metric("🏁 Batterie", f"{res['soc_final']:.1f}%")
